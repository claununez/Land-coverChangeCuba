...
# Reading the layer
layer1985 <- raster("cuba_1985_rfL5_majority1.tif")
layer2020 <- raster("cuba_2020_rfL8_majority1.tif")
...
# Statistics (All Cuba)
val_1985 <- na.omit(layer1985[])
val_2020 <- na.omit(layer2020[])
val_both <- na.omit(both[])

count1985 <- table(val_1985)
count2020 <- table(val_2020)
countboth <- table(val_both)

write.csv(count1985, "count1985.csv")
write.csv(count2020, "count2020.csv")
write.csv(countboth, "countboth.csv")

# Protecting areas in Cuba
prot_areas <- readOGR("AreasProtegidasSNAP2014_2020_region.shp")

prot_areas <- spTransform(prot_areas, layer1985@crs)

prot_ar_layer1985 <- mask(crop(layer1985, prot_areas), prot_areas)
prot_ar_layer2020 <- mask(crop(layer2020, prot_areas), prot_areas)
prot_ar_both <- mask(crop(both, prot_areas), prot_areas)

writeRaster(prot_ar_layer1985, "prot_ar_layer1985_1", format = "GTiff", overwrite = T)
writeRaster(prot_ar_layer2020, "prot_ar_layer2020", format = "GTiff", overwrite = T)
writeRaster(prot_ar_both, "prot_ar_both", format = "GTiff")
...
write.csv(prot_count1985, "prot_count1985.csv")
write.csv(prot_count2020, "prot_count2020.csv")
write.csv(prot_countboth, "prot_countboth.csv")

# Considering Omphalea potential distribution
# Reading Omphalea raster
omp <- raster("modelo_omphalea.tif") 
omp[omp[] == 0] <- NA
omp1 <- rasterToPolygons(omp, dissolve = TRUE)
writeOGR(omp1, dsn = ".", layer = "omp1", driver = "ESRI Shapefile")

omp_layer1985 <- mask(crop(layer1985, omp1), omp1)
omp_layer2020 <- mask(crop(layer2020, omp1), omp1)
omp_both <- mask(crop(both, omp1), omp1)

writeRaster(omp_layer1985, "omp_layer1985", format = "GTiff", overwrite = T)
writeRaster(omp_layer2020, "omp_layer2020", format = "GTiff", overwrite = T)
writeRaster(omp_both, "omp_both", format = "GTiff")
...
write.csv(omp_count1985, "omp_count1985.csv")
write.csv(omp_count2020, "omp_count2020.csv")
write.csv(omp_countboth, "omp_countboth.csv")
...
writeRaster(omp_prot_ar_layer1985, "omp_prot_ar_layer1985", format = "GTiff",
            overwrite = T)
writeRaster(omp_prot_ar_layer2020, "omp_prot_ar_layer2020", format = "GTiff",
            overwrite = T)
writeRaster(omp_prot_ar_both, "omp_prot_ar_both", format = "GTiff")
...
write.csv(omp_prot_count1985, "omp_prot_count1985.csv")
write.csv(omp_prot_count2020, "omp_prot_count2020.csv")
write.csv(omp_prot_countboth, "omp_prot_countboth.csv")
